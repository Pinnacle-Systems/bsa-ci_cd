...
export async function get__Advance(req, res) {
  ...
  try {
    const result = await prisma_Connector.advanceRequest.findMany({
      where: hod
...
export async function get_advance(req, res) {
  ...
  try {
    ...
    WHERE A.PTRANSACTION=:adv AND B.COMPCODE=:COMPCODE AND A.IDCARD=:IDCARD order by A.DOCDATE  DESC ) where  ROWNUM=1`,
      { COMPCODE, IDCARD: Idcard, adv: advanceType },
    );
...
export async function add_requestAdvance(req, res) {
  ...
  if (!req?.body?.data) {
    return res
      .status(400)
      .json({ status: 0, message: "Missing required data in request body" });
  }
  ...
  try {
    ...
    if (!connection) {
      return res
        .status(500)
        .json({ status: 0, message: "Database connection failed" });
    }
    ...
    var PTrans =
      result.advtype == "Salary Advance"
        ? result.advtype?.split(" ")[1]
        : result.advtype;
    const auvitData = {
      COMPCODE: compid,
    ...
    const sql = `INSERT INTO HRMPAYTRANS (
...
export async function requestLoan_Approval(req, res) {
  ...
  const connection = await getConnection(res);
  try {
    const result = await prisma_Connector.advanceRequest.update({
      where: { docid },
    ...
    if (result?.id) {
      ...
      const result_auvit = await connection.execute(sql, {
        approvalStatus: String(p_app).toUpperCase(),
        docid,
      });
      if (result_auvit?.rowsAffected > 0) {
        await prisma_Connector?.advanceRequest
          ?.update({ where: { docid: result?.docid }, data: { update: 1 } })
          .then(async () => {
            await connection.commit();
        ...
        await prisma_Connector?.advanceRequest?.delete({
          where: { docid: docid },
        });
...
export async function get_Paycode(req, res) {
  ...
  try {
    const result = await connection.execute(
      `SELECT C.HRPAYCOMPONENTSID "paycode", C.PAYCODE ,C.PAYDESC
