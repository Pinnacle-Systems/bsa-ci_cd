...
import {
  monthWiseDataSupplierReceivables,
} from "../queries/poData.js";
...
export async function get(req, res) {
  ...
  try {
    ...
    const filterDateWiseQ1 =
      startDate && endDate
        ? `AND Q1.DOCDate BETWEEN TO_DATE('${startDate}', 'YYYY-MM-DD') AND TO_DATE('${endDate}', 'YYYY-MM-DD')`
        : "";
    const filterDateWiseQ2 =
      startDate && endDate
        ? `AND Q2.DOCDate BETWEEN TO_DATE('${startDate}', 'YYYY-MM-DD') AND TO_DATE('${endDate}', 'YYYY-MM-DD')`
        : "";
    const filterDateWiseQ3 =
      startDate && endDate
        ? `AND Q3.DOCDate BETWEEN TO_DATE('${startDate}', 'YYYY-MM-DD') AND TO_DATE('${endDate}', 'YYYY-MM-DD')`
        : "";
    const filterDateWiseQ4 =
      startDate && endDate
        ? `AND Q4.DOCDate BETWEEN TO_DATE('${startDate}', 'YYYY-MM-DD') AND TO_DATE('${endDate}', 'YYYY-MM-DD')`
        : "";
    const filterDateWiseM =
      startDate && endDate
        ? `AND M.DOCDate BETWEEN TO_DATE('${startDate}', 'YYYY-MM-DD') AND TO_DATE('${endDate}', 'YYYY-MM-DD')`
        : "";

    //year filter
    const fltrYearData = finYearData
      ? JSON.parse(finYearData).map((item) => `'${item}'`)
      : [];
    const fltrYearClauseQ1 =
      fltrYearData.length > 0 ? `AND Q1.FINYR IN (${fltrYearData})` : "";
    const fltrYearClauseQ2 =
      fltrYearData.length > 0 ? `AND Q2.FINYR IN(${fltrYearData})` : "";
    const fltrYearClauseQ3 =
      fltrYearData.length > 0 ? `AND Q3.FINYR IN(${fltrYearData})` : "";
    const fltrYearClauseQ4 =
      fltrYearData.length > 0 ? `AND Q4.FINYR IN(${fltrYearData})` : "";
    const fltrYearClauseM =
      fltrYearData.length > 0 ? `AND M.FINYR IN(${fltrYearData})` : "";
    //month filter
    const monthFilter = filterMonth
      ? JSON.parse(filterMonth).map((item) => `'${item}'`)
      : [];
    console.log();
    const fltrMonthQ1 =
      monthFilter.length > 0
        ? `AND EXTRACT(MONTH FROM Q1.DOCDATE)IN (${monthFilter})`
        : "";
    const fltrMonthQ2 =
      monthFilter.length > 0
        ? `AND EXTRACT(MONTH FROM Q2.DOCDATE)IN (${monthFilter})`
        : "";
    const fltrMonthQ3 =
      monthFilter.length > 0
        ? `AND EXTRACT(MONTH FROM Q3.DOCDATE)IN (${monthFilter})`
        : "";
    const fltrMonthQ4 =
      monthFilter.length > 0
        ? `AND EXTRACT(MONTH FROM Q4.DOCDATE)IN (${monthFilter})`
        : "";
    const fltrMonthM =
      monthFilter.length > 0
        ? `AND EXTRACT(MONTH FROM M.DOCDATE)IN (${monthFilter})`
        : "";
    // supplier filter
    const suppFilter = filterSupplier
      ? JSON.parse(filterSupplier).map((item) => `'${item}'`)
      : [];
    const filterSupp =
      suppFilter.length > 0 ? `AND SUPPLIER IN (${suppFilter})` : "";
    // Article id filter
    const articleIdFilter = filterArticleId
      ? JSON.parse(filterArticleId).map((item) => `'${item}'`)
      : [];
    const filterArtId =
      articleIdFilter.length > 0
        ? ` AND ARTICLEID IN (${articleIdFilter})`
        : "";
