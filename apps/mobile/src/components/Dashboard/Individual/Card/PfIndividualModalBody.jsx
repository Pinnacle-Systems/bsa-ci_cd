...
function PfIndividualModalBody({CurrentPFAmt, CurrentYearandMonth, UserId}) {
  ...
  const {data: esiPfdata, error: err} = useGetMonthESIPFQuery({
    params: {
      Idcard: UserId,
      payperiod:
        pfMonth && pfyear ? pfMonth + ' ' + pfyear : CurrentYearandMonth,
      type: mory,
    },
  });
  ...
  const esidataOnMonth =
    mory == 'Y'
      ? esiPfdata?.data?.reduce(
          function (acc, obj) {
            return {pf: acc.pf + obj.pf};
          },
          {pf: 0},
        )
      : esiPfdata?.data
    ...
    <View>
      ...
      <View style={{flexDirection: 'row', width: '100%', gap: 10}}>
        <CustomDropdownSelect
          ...
          isyear={true}
          selectedValue={pfyear}
          setSelectedValue={setpfyear}
        />
        {mory == 'M' && (
          <CustomDropdownSelect
            placeholder={'select Month'}
            ismonth={true}
            selectedValue={pfMonth}
            setSelectedValue={setpfMonth}
          />
        )}
      </View>
      <View style={EsiandPFBodyStyle?.EsipfContainer}>
        ...
        <CustomText
          ...
          {' '}
          {NumbertoCurrency(esidataOnMonth?.pf || 0) || CurrentPFAmt}
        </CustomText>
      </View>
    </View>
  );
